...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for part in 1h/{0..5}/{clean,other} 9h/{clean,other}; do
        ...
        data_part=$(local/local_for_librilight10/make_absolute.sh ${src}/${part})
        data_new_path="data/${train_set}_${dataname}"
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    ./utils/combine_data.sh \
        data/${train_set} data/${train_set}_1h_{0..5}_{clean,other} data/${train_set}_9h_{clean,other}
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    if [ ! -e "${LIBRISPEECH}/LibriSpeech/LICENSE.TXT" ]; then
        ...
        for part in dev-clean test-clean dev-other test-other; do
            local/local_for_librilight10/download_and_untar_eval.sh "${LIBRISPEECH}" "${ls_data_url}" "${part}"
        done
    else
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for part in dev-clean test-clean dev-other test-other; do
        ...
        new_part=${part//dev/valid}
        local/local_for_librilight10/data_prep_eval.sh ${LIBRISPEECH}/LibriSpeech/${part} data/Librispeech_${new_part//-/_}
    done
    utils/combine_data.sh --extra_files utt2num_frames data/Librispeech_valid data/Librispeech_valid_clean data/Librispeech_valid_other
fi
