...
if [ -d ${raw_data} ];then
    ...
    find $raw_data -type f -name "*.flac" > $data/data_all/wavpath
    awk -F'/' '{print $NF}' $data/data_all/wavpath | sed 's:\.flac::g' > $data/data_all/uttlist
    paste $data/data_all/uttlist $data/data_all/wavpath > $data/data_all/wav.scp

    # faster than for in shell
    python local/local_for_librispeech360_speaker_adaptation/preprocess.py $data/data_all/wav.scp $data/data_all/trans $data/data_all/utt2spk
...
echo "local/local_for_librispeech360_speaker_adaptation/data_prep.sh succeeded"
exit 0;
